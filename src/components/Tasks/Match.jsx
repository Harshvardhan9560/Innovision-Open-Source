...
export default function Match({ task, roadmapId, chapterNumber }) {
    ...
    const updateLines = () => {
        ...
        matches.forEach((rightIndex, leftIndex) => {
            if (
                rightIndex !== -1 &&
                leftRefs.current[leftIndex] &&
                rightRefs.current[rightIndex]
            ) {
                const leftRect =
                    leftRefs.current[leftIndex].getBoundingClientRect();
                const rightRect =
    ...
    const getLeftItemColor = (index) => {
        ...
        if (matches[index] !== -1)
            return "bg-blue-200/80 border-blue-400/80 dark:bg-blue-900/50 dark:border-blue-700";
                    ...
                    <svg className="absolute top-0 left-0 w-full h-full pointer-events-none z-10">
                        {lines.map((line, index) => {
                            ...
                            const controlPointX2 =
                                line.from.right +
                                (line.to.left - line.from.right) * 0.6;
                        ...
                        <div className="space-y-4">
                            {task.terms.lhs.map((term, index) => (
                                <div
                                    ...
                                    className={`p-3 border rounded-lg w-full cursor-pointer transition-colors ${getLeftItemColor(
                                        index
                                    )}`}
