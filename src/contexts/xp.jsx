...
export const XpProvider = ({ children }) => {
    ...
    const getXp = useCallback(async () => {
        ...
        try {
            ...
            console.log("XP Context - Fetched data:", data);
            
            if (data && typeof data.xp === 'number') {
                const xpDiff = data.xp - xp;
                if (xpDiff > 0 && xp > 0) { // Only show animation if xp was already set
                    setChanged(xpDiff);
                    change();
                }
    ...
    return (
        <xpContext.Provider value={{ getXp, awardXP, xp, show, changed }}>
            {children}
        </xpContext.Provider>
    );
};
